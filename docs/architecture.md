...
correctness mechanism, not a public transaction API.
...
| --- | --- | --- |
| Common owned terms and snapshot semantics | Not met | Value equality is compatible, but the reasoner owns an interned triple store and the SPARQL adapter makes a second owned quad snapshot.  See ADR 0002. |
...
| Minimal API from existing use cases | Not met | Reasoner needs indexed default-graph triple closure; SPARQL needs graph-scoped read scans.  The adapter is a copying, linear default-graph view and named-graph support is not shared. |
