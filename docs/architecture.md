...
| --- | --- | --- |
| Common owned terms and snapshot semantics | Partial | Released reasoner v0.3.0 can transfer its owned terms and indexes into an immutable SPARQL Snapshot. The experimental `odin-graph` commit pinned by Garden has its own owned frozen view and optional SPARQL adapter, but `Memory_Dataset` and the reasoner still own separate representations. See ADR 0002. |
